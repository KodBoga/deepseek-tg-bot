...
bot.on("text", async (ctx) => {
  ...
  // Кнопка "Акция" — внутри шага уточнения записи
  if (raw === "Акция") {
    return ctx.replyWithPhoto(
      ...
      },
      {
        caption:
          ...
          "В стоимость входит:\n" +
          "• консультация любого врача‑стоматолога\n" +
          "• компьютерная 3D‑диагностика (КЛКТ)\n\n" +
  ...
  // Выбор дня (для любой услуги, когда дошли до записи)
...
Комментарий: ${lead.comment}
  ...
  // Консультация — просто копим контекст, а приглашение на приём будет общим ниже
  ...
  if (!state.phone) {
    ...
    // Если уже приглашали, но человек продолжает писать текст — переходим к телефону
    state.waitingForPhone = true;
    return ctx.reply("Чтобы записать вас на приём, напишите, пожалуйста, номер телефона для связи.");
  }
